...
function ejercicio2 (){
    if(numAlumn.value) {
        if(numAlumn.value >= 2 && numAlumn.value <= 20){
...
function ejercicio3(){
    ...
    if(iguales == true) resultados3.innerHTML = `Resultado: ${true}`;
    else resultados3.innerHTML = `Resultado: ${true}`;


    // if(strings[0].value.toLowerCase().includes(strings[1].value.toLowerCase())){
    //     resultados3.innerHTML = `Resultado: ${true}`;
